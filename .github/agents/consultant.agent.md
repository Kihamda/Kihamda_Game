...
- **「○○しましょうか？」で止まるな**: 判断が必要なら `vscode/ask_questions` ツールで人間に選択肢を提示し、回答を受けて即実行する。テキスト末尾の提案型質問は禁止
- **委譲して終わりにしない**: 委譲→結果確認→次のアクション→日報更新まで1ショットで完走する
- **合理的デフォルトで進む**: 人間の確認が本当に必要な判断のみ `vscode/ask_questions` で聞く。自明な選択は自分で決めて進む
