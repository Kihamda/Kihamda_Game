...
## Portal SSG Plugin (`plugins/portal-ssg.ts`)

ビルド時に以下を自動生成:
- ポータルのSEO用静的ページ (`dist/.ssg/`)
- `sitemap.xml` (全ゲームのURL含む)
- `_redirects` / `_headers` (SPA対応)

vite.config.ts でプラグインとして登録済み。

## 静的ホスティングでのSPAルーティング

XServer Static はSPAルーティングに対応するため、以下のファイルが `dist/` に必要:

- `_redirects`: `/* /index.html 200` でSPAフォールバック
- `_headers`: キャッシュ制御ヘッダー

これらは `plugins/portal-ssg.ts` が自動生成する。

## ゲーム登録メカニズム (`src/games/registry.ts`)

`import.meta.glob("../../games/*/src/App.tsx")` で全ゲームを自動検出。

パターン: `games/[game-id]/src/App.tsx`
→ game-id を抽出して遅延ロード用マップを生成

新ゲーム追加時、このファイルの編集は不要（glob が自動検出）。
